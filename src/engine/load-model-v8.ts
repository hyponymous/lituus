/**
 * Read a KataGo `.bin` network into a `ParsedKataGoModelV8`.
 *
 * Upstream: `src/engine/katago/loadModelV8.ts` in web-katrain
 * (https://github.com/Sir-Teo/web-katrain), MIT. Lifted; the only changes are
 * the import specifiers, which carry `.ts` extensions here (see CLAUDE.md) and
 * take the model types from `model-types.ts` rather than from the TensorFlow.js
 * graph module, so this file stays free of TensorFlow.js.
 *
 * The layer order is the file format, not a choice: every `read` below has to
 * happen in exactly this sequence or the stream desynchronizes and the next
 * token is garbage. Treat it as a transcription of KataGo's own serializer.
 */
