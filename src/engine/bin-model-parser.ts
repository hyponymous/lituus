/**
 * Tokenizer for KataGo's native `.bin` model format.
 *
 * Upstream: `src/engine/katago/binModelParser.ts` in web-katrain
 * (https://github.com/Sir-Teo/web-katrain), MIT. Lifted essentially verbatim;
 * see `docs/reuse-notes.md` for the divergences.
 *
 * The format is ASCII tokens — names, ints, floats — interleaved with blocks
 * of raw little-endian float32 introduced by an `@BIN@` marker. Nothing here
 * touches TensorFlow.js or the DOM, which is deliberate: `docs/design-ai-scoring.md`
 * §10 requires the pure half of `engine/` to run under `node --test` with no
 * bundler.
 */
